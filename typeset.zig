...
pub fn Typeset(comptime name: []const u8, comptime types: anytype) type {
    ...
    return struct {
        ...
        pub fn call(data_ptr: anytype, comptime fn_name: []const u8, args: anytype) ReturnType(U, fn_name, name) {
                ...
                inline else => |*val_ptr| blk: {
                    ...
                    switch (@typeInfo(val_type)) {
                        .Pointer => |type_info| {
                            ...
                            const fn_ptr = &@field(val_type, fn_name);
                            break :blk @call(.auto, fn_ptr, .{val_ptr} ++ args);
                        },
                    }
                },
...
fn ReturnType(comptime U: type, comptime fn_name: []const u8, comptime typeset_name: []const u8) type {
    const type_info = @typeInfo(U);
    if (type_info != .Union) {
        @compileError("Expected " ++ @typeName(U) ++ " to be a union.");
    }
        ...
        switch (@typeInfo(T)) {
            .Struct, .Union, .Enum, .Opaque => {
                if (@hasDecl(T, fn_name)) {
                    ...
                    return @typeInfo(@TypeOf(fn_t)).Fn.return_type.?;
                }
            },
            .Pointer => |ptr| {
                const T2 = ptr.child;
            ...
            else => @compileError("Invalid type for Typeset method call: " ++ @typeName(T)),
        }
    }
